...
・義手をはじめとする多指ハンドロボットの研究において，VAEは多次元の制御入力を低次元に圧縮するため利用される\ref{}\ref{}．

本研究では変分オートエンコーダの学習によって獲得した潜在空間を強化学習における行動空間として利用することで効率的な強化学習を実現する．


VAEでは行動ベクトルを低次元に圧縮可能
<- VAEはニューラルネットを用いた非線形のフレームワークで，誤差を小さくできる．
...
% 更に書くべき事項：確率的方策を実行したのか初期値をランダムにずらしたのか．
